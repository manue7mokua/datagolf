...
elif /usr/bin/python3 -c "import fastapi, pandas" >/dev/null 2>&1; then
  python_bin="/usr/bin/python3"
elif python3 -c "import fastapi, pandas" >/dev/null 2>&1; then
  python_bin="python3"
else
  echo "FastAPI and pandas test dependencies are not installed in the same Python. Install apps/api/requirements.txt or set DATAGOLF_E2E_PYTHON." >&2
  exit 1
fi

if ! "$python_bin" -c "import fastapi, pandas" >/dev/null 2>&1; then
  echo "DATAGOLF_E2E_PYTHON must point to a Python with FastAPI and pandas installed." >&2
  exit 1
fi
